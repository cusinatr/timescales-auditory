"""
Plot correlation of baseline timescales with auditory evoked responses latencies.

The script reads data contained in the csv file specified by data_name and resp_name,
where the timescale and latencies for each channel are already stored.
...
df_data = get_resp_params(df_resp, df_tau, ["tau"])
...
df_test_onset = run_R_test_corr(source_path, df_data, "tau", "onset", save_path)
df_test_peak = run_R_test_corr(source_path, df_data, "tau", "peak", save_path)
...
save_fig(
    fig, path.join(base_path, save_dir), "Corr_onset_tau_all", format=save_format,
)
...
ax.set_ylim(0, 650)
save_fig(
    fig, path.join(base_path, save_dir), "Corr_peak_tau_all", format=save_format,
)
...
for i, reg in enumerate(df_tau.region.unique()):

    # Onset
    fig, ax = plt.subplots(1, 1, figsize=[5, 5])
    ax = plot_corr.plot(
        ax,
        df_data.tau[df_data.region == reg],
    ...
    ax.set_ylim(0, 650)
    save_fig(
        fig, path.join(base_path, save_dir), "Corr_peak_tau_" + reg, format=save_format,
    )
